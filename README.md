...
Node version is pinned to **22** (`.nvmrc` + `engines` in `package.json`) —
the current Wrangler / OpenNext deploy needs Node 22+. If you use nvm:

```bash
nvm install 22 && nvm use 22
npm install          # re-run after switching Node versions
```

Do the same inside WSL (see §1.1). On Cloudflare, set `NODE_VERSION = 22`
as a build variable (see §9.1).
...
**Node version — required.** The current Wrangler needs **Node 22+**; Cloudflare's
build image still defaults to Node 20 (`Wrangler requires at least Node.js
v22.0.0. You are using v20.x`). Fix it by adding a build variable in
**Workers & Pages → the project → Settings → Variables and Secrets** (Build
variables), or in **Settings → Build → Build variables**:

| Variable       | Value |
| -------------- | ----- |
| `NODE_VERSION` | `22`  |

`.nvmrc` (`22`) in the repo is also read by the build image, but the
`NODE_VERSION` variable is the reliable switch — set it. Redeploy after adding it.
...
-- Files sent with an admin email. The file itself lives in the private
-- `email-attachments` Storage bucket; this row links it to the sent email.
create table sent_email_attachments (
  id uuid primary key default gen_random_uuid(),
  sent_email_id uuid references sent_emails(id) on delete cascade,
  filename text not null,
  storage_path text not null,   -- path in the email-attachments bucket
  content_type text,
  size_bytes int,
  created_at timestamptz not null default now()
);
...
3. **Create the Storage bucket for attachments.** Dashboard → **Storage → New
   bucket** → name it `email-attachments`, leave **Public** *off* (private).
   ...
   No RLS policies are needed: the server uses the secret key (which bypasses
   RLS), and files are only ever reached through short-lived signed URLs
   generated by the protected `/api/admin/attachments/[id]` route.

4. Get your project's API values:
...
**Attachments.** Both the Inbox reply box and Compose let you attach files
(PDF, images, Word, Excel — **max 5 MB each**, 12 MB total, 5 files). They
are read in the browser as base64 and passed to Resend as `attachments`.
Oversized or unreadable files are rejected with a clear message and the
typed text is kept so you can retry.

On a successful send, each file is also uploaded to the private
`email-attachments` Storage bucket (path `sent/{sent_email_id}/{filename}`)
and recorded in `sent_email_attachments`. The **Sent** reading pane shows
these as chips with a **Download** button, which opens a 60-second signed
URL from the protected `/api/admin/attachments/[id]` route — the bucket is
never public. A storage upload that fails is logged and skipped; the email
(already sent) is unaffected. Emails sent before this feature simply show no
chips.

Like every send, **attachments only reach a real customer inbox once your
own domain (e.g. `bilic.com`) is verified in Resend — see §10.3.**
